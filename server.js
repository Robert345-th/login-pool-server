...
app.get('/', (req, res) => {
    ...
    const inUseAccounts = accounts.filter(acc => acc.status === 'IN-USE').length;
    ...
    let accountCards = accounts.map(acc => `
        <div style="background:#1e293b; padding:15px; margin:10px 0; border-radius:8px; display:flex; justify-content:space-between; align-items:center; border: 1px solid #334155;">
            ...
            <span style="
                background:${acc.status === 'FREE' ? '#065f46' : acc.status === 'IN-USE' ? '#1e3a8a' : '#7e22ce'};
                color:${acc.status === 'FREE' ? '#34d399' : acc.status === 'IN-USE' ? '#93c5fd' : '#e9d5ff'};
                padding:6px 12px; border-radius:6px; font-weight:bold; font-size:0.85em; text-transform:uppercase;">
                ${acc.status}
            </span>
        </div>
    `).join('');
    ...
    <body>
        ...
        <div class="stats-bar">
            ...
            <span style="color:#34d399;">Free: ${freeAccounts}</span> | 
            <span style="color:#93c5fd;">In-Use: ${inUseAccounts}</span> | 
            <span style="color:#e9d5ff;">Locked: ${lockedAccounts}</span>
        </div>
